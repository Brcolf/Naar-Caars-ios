-- Fix badge counts: restrict to the current user's conversations only.
-- 20260216_0001 counted unread messages across ALL conversations (no conversation_participants
-- join), so badges showed every unread in the DB. This restores the 102/107 logic: only count
-- unread in conversations where the user is a participant; cleanup and conversation_details
-- are scoped the same way so badges dismiss correctly.
