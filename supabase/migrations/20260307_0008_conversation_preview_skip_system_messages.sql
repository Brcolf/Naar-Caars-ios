-- Fix: conversation list preview should show the last non-system message
-- System messages (member added/removed/left, name changed, etc.) should not
-- appear as the conversation preview in the list view.
...
BEGIN
    ...
    WITH conversation_ids AS (
        ...
        FROM conversations
        WHERE conversations.created_by = p_user_id
    ),
    ...
    last_messages AS (
        ...
        WHERE m.conversation_id IN (SELECT cr.id FROM conversation_rows cr)
          AND (m.message_type IS NULL OR m.message_type != 'system')
