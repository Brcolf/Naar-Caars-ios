-- Security hardening: validate that authenticated callers can only query their
-- own badge counts.  The function already resolves v_user_id via
-- coalesce(auth.uid(), p_user_id), but a malicious caller could pass someone
-- else's UUID as p_user_id while authenticated.  This migration adds a guard
-- that raises an exception when auth.uid() and p_user_id are both present and
-- do not match.
