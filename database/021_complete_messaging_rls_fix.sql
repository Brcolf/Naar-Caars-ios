-- Complete RLS fix for messaging workflows
-- This fixes the recursion issue AND supports all messaging workflows:
-- 1. Request-based conversations (ride/favor)
-- 2. Direct messages
-- 3. Group chats
-- 4. Adding participants to conversations
...
-- ============================================================================
-- Step 2: Drop function (we'll recreate it if needed, but try without first)
-- ============================================================================

DROP FUNCTION IF EXISTS public.is_conversation_participant(UUID, UUID) CASCADE;

-- ============================================================================
-- Step 3: Create simplified conversation_participants SELECT policy
-- Only show user's own participation rows - NO RECURSION
-- ============================================================================
...
-- - Request creator (if conversation is linked to a request they created)
-- NOTE: Removed "existing participants can add" to avoid recursion
-- For group chats, conversation creator can add participants
-- ============================================================================
...
-- ============================================================================
-- Step 6: Create conversations SELECT policy
-- Users can see conversations where:
-- - They are the creator
-- - They are a participant (checked via direct query, no function)
-- ============================================================================
...
CREATE POLICY "conversations_select_participant" ON public.conversations
  FOR SELECT USING (
    ...
    created_by = auth.uid()
    OR
    -- OR user is a participant (direct query, no recursion)
    EXISTS (
      SELECT 1 
...
-- ============================================================================
-- Step 9: Create messages SELECT policy
...
-- ============================================================================
-- Step 10: Create messages INSERT policy
-- Users can send messages as themselves in conversations they're in
-- ============================================================================
...
-- 1. SELECT * FROM conversations; (should show user's conversations)
-- 2. SELECT * FROM conversation_participants; (should show user's participations)
-- 3. Try creating a conversation and adding participants
