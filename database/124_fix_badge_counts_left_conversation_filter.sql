-- Fix get_badge_counts to exclude messages from conversations the user has left.
-- Three queries inside the function join conversation_participants but don't
-- filter on left_at, so messages sent after a user leaves still count as unread.
-- Also cleans up an orphaned review_request notification.
...
begin
    ...
    -- Messages: total unread for user only in active conversations
    ...
    -- when there are no unread messages in that conversation (user must be active participant).
...
-- Clean up orphaned review_request notification for Brendan.
-- The ride (55a4e443) has claimed_by=null and reviewed=false with 0 reviews,
-- making this notification permanently stuck and unactionable.
UPDATE public.notifications
SET read = true
WHERE id = '895768e8-2dbf-479c-92e2-2f3f86e81568';
